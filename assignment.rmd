...
```{r}
interval.average<- summarise(group_by(mydata, interval), mean=mean(steps, na.rm=T))
plot(y=interval.average$mean, x=interval.average$interval, type="l", main="Daily activity patterns", xlab="Time interval (minutes)", ylab="Average steps")
```

The time interval with the most steps, on average:
```{r}
mydata[which(interval.average$mean==max(interval.average$mean)), "interval"]
...
backfilleddata$steps<- ifelse(is.na(backfilleddata$steps), backfilleddata$mean, backfilleddata$steps) # replace the "steps" value with the mean for all NAs.
